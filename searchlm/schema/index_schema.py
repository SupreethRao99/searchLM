...
class IndexSchema:
    ...
    def build(self) -> tantivy.Schema:
        ...
        if self._schema is None:
            ...
            schema_builder.add_text_field(FIELD_DOC_ID, stored=True, tokenizer_name="raw")
            
            # Title field - stored with English stemming for better search
            schema_builder.add_text_field(FIELD_TITLE, stored=True, tokenizer_name="en_stem")
            
            # Text content field - stored with English stemming
            schema_builder.add_text_field(FIELD_TEXT, stored=True, tokenizer_name="en_stem")
            
            # Dataset identifier - stored as-is
            schema_builder.add_text_field(FIELD_DATASET, stored=True, tokenizer_name="raw")
            
            # Source ID - original document ID from source dataset
            schema_builder.add_text_field(FIELD_SOURCE_ID, stored=True, tokenizer_name="raw")
